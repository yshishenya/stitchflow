...
> Turn plain-language product briefs into Stitch projects, UI directions, design systems, prototypes, Tailwind-friendly HTML, and screenshots in minutes.
>
> Prompt -> Stitch project -> prototype/code handoff -> local evidence artifacts.
...
- create and reuse `DESIGN.md` design systems
- keep final multi-screen flows in Stitch before implementation
...
5. open the generated HTML and screenshot artifacts from the local `runs/` folder, or use the Stitch project as the source for prototype/code export
...
```text
Use $stitchflow to build a full multi-screen Stitch prototype from this PRD, upload DESIGN.md as the project design system, link the screens in Stitch, and export a local evidence checkpoint.
```
...
Re-running the installer with `--force` preserves the existing toolkit `.env`
and `runs/` artifacts.
...
- `DESIGN.md` upload and design-system creation helpers
- project-wide screen export checkpoints
...
- a Stitch project/prototype that remains the design source of truth
...
- local HTML, screenshots, and run artifacts
- `DESIGN.md` design-system helpers
- local code/assets downloads for screen-level handoff
- project-level export checkpoints for multi-screen review
...
npm run generate -- --prompt "A cinematic product homepage" --timeout-ms 900000 --retries 2
...
- turn a PM brief into a Stitch project, HTML, and screenshots for review
- iterate on a dashboard without opening Figma
- create visual consistency across a full flow with `DESIGN.md`
- download screen-level `code.html` plus referenced styles/images for handoff
...
- `html-url.txt`
- `image-url.txt`
- `project.json` for project-wide exports
...
- exports clean HTML and screenshots programmatically through the SDK
- supports Stitch project/prototype handoff when final code export is done in the Stitch web UI
...
- Stitch-проектом/прототипом как источником дизайн-решения
...
- helper-команды для `DESIGN.md` и дизайн-систем
- локальное скачивание code/assets для screen-level handoff
- project-wide export checkpoints для ревью всех экранов
...
- превратить PM brief в Stitch-проект, HTML и скриншоты для ревью
- итерировать dashboard без Figma
- удерживать визуальную систему через `DESIGN.md`
- скачивать screen-level `code.html` с локальными стилями/изображениями
...
- `html-url.txt`
- `image-url.txt`
- `project.json` для project-wide export
...
- поддерживает Stitch project/prototype handoff, когда финальный export делается через Stitch web UI
